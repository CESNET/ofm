...
class RecorderReader(object):
    ...
    def _det_start(self,mfb):
        ...
        reg = 0
        pos = 0
        sof_vld = (mfb["VLD"] >> (1+self.regions)) & long('1' * self.regions,2)
        if sof_vld != 0:
            ...
            pos = (mfb["SOF_POS"] >> (reg*self.sof_elem_width)) & long('1'*self.sof_elem_width,2)
        return (det,reg,pos)
    ...
    def _det_end(self,mfb_word):
        ...
        reg = 0
        pos = 0
        eof_vld = (mfb_word["VLD"] >> 1) & long('1' * self.regions,2)
        if eof_vld != 0:
            # The end is detected
            det = True
            ...
            pos = (mfb_word["EOF_POS"] >> (reg*self.eof_elem_width)) & long('1'*self.eof_elem_width,2)
        return (det,reg,pos)
