...
try:
    ...
    l = logging.getLogger("modelsim")
    l.warn("can't load modelsim interpreter handle.")

def cmd(command):
    x = __lib.mti_Cmd(command.encode())
...
def add_wave(*args, **kwargs):
    ...
    if 'group' in kwargs:
        ...
        # Nested grouping also possible, just use more -group switches. This should be done by new kwarg, for example. cmd_prefix='-group base_group_name'
    if 'expand' in kwargs:
        params += f' -expand'# "{kwargs["expand"]}"'
    if 'label' in kwargs:
